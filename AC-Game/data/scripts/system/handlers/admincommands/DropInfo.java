...
public class DropInfo extends AdminCommand {
	...
	public void execute(Player player, String... params) {
		...
		for (DropGroup dropGroup: npcDrop.getDropGroup()){
			...
			// TF keys from fatties
			if (dropGroup.getGroupName().equals("NAMED_KEY_ALONE") && player.getAccessLevel()==0)
				continue;

			PacketSendUtility.sendMessage(player, "DropGroup: "+ dropGroup.getGroupName());
			for (Drop drop : dropGroup.getDrop()){
				if (player.getAccessLevel()==0)	// normal players don't get the drop rate, to avoid complaints
					PacketSendUtility.sendMessage(player, "[item:" + drop.getItemId() + "]");
				else
